...
#[allow(dead_code)]
impl AccountBot {
    pub async fn get_token(&self, db: &mut Conn, user_id: &UserId) -> Result<String, mysql_async::Error> {
        // clear old tokens
    ...
    pub async fn manage_account(&self, db: &mut Conn, ctx: Context, msg: Message) -> Result<(), mysql_async::Error> {
        let user_id = msg.author.id;
        let token = self.get_token(db, &user_id).await?;
...
This link will only work for ten minutes. Do *not* share this link.",
    ...
    pub async fn whois(&self, db: &mut Conn, ctx: Context, msg: Message) -> Result<(), mysql_async::Error> {
        ...
        if msg.content.starts_with(r"<@") && msg.content.ends_with(r">") {
            ...
            if let Ok(_) = cap.parse::<u64>() {
                if let Some(account) = self.get_account_id_str(db, cap.as_str()).await? {
    ...
    pub async fn get_account_id(&self, db: &mut Conn, user: UserId) -> Result<Option<String>, mysql_async::Error> {
        let result: Option<String> = db.query_first(
    ...
    pub async fn get_account_id_str(&self, db: &mut Conn, user: &str) -> Result<Option<String>, mysql_async::Error> {
        let result: Option<String> = db.query_first(
